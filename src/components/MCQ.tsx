...
const MCQ = ({ game }: Props) => {
    ...
    // Set startTime once when first question becomes available
    React.useEffect(() => {
        if (!startTime && game.questions.length > 0) {
            setStartTime(new Date());
        }
    }, [game.questions.length, startTime]);
    ...
    }, [checkAnswer, questionIndex, game.questions.length, endGame, options, selectedChoice]);
